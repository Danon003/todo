...
public class ReportServiceImpl implements ReportService {
    ...
    @Override public byte[] generateReport(ReportRequestDTO request) throws IOException {
        ...
        if ("excel".equalsIgnoreCase(request.getFormat()) || request.getFormat() == null) {
            ...
            log.error("Неподдерживаемый формат отчета: {}", request.getFormat());
    ...
    private byte[] generateExcelReport(ReportRequestDTO request) throws IOException {
        log.debug("Генерация Excel отчета");
        ...
        String reportTitle = getReportTitle(request);
        ...
        switch (request.getReportType()) {
            ...
            case "COMPREHENSIVE":
                ...
                break;
            default:
                log.error("Неизвестный тип отчета: {}", request.getReportType());
        ...
        log.info("Excel отчет успешно сгенерирован: {}, размер: {} bytes", reportTitle, outputStream.size());
    ...
    private void generateStudentProgressReport(Sheet sheet, ReportRequestDTO request,
        ...
        log.debug("Генерация отчета по прогрессу студентов");
        int rowNum = startRow;
        ...
        log.debug("Найдено {} назначений для отчета", assignments.size());
    ...
    private void generateTaskStatisticsReport(Sheet sheet, ReportRequestDTO request,
                                              CellStyle headerStyle, CellStyle dataStyle, int startRow) {
        log.debug("Генерация отчета по статистике задач");
        int rowNum = startRow;
        Row headerRow = sheet.createRow(rowNum++);
        ...
        log.debug("Найдено {} задач для отчета", tasks.size());
    ...
    private void generateGradesOverviewReport(Sheet sheet, ReportRequestDTO request,
                                              CellStyle headerStyle, CellStyle dataStyle, int startRow) {
        log.debug("Генерация отчета по оценкам");
        int rowNum = startRow;
        Row headerRow = sheet.createRow(rowNum++);
        ...
        log.debug("Найдено {} назначений с оценками", assignments.size());
    ...
    private void generateComprehensiveReport(Sheet sheet, ReportRequestDTO request,
                                             CellStyle headerStyle, CellStyle dataStyle, int startRow) {
        log.debug("Генерация комплексного отчета");
    ...
    private byte[] generateDocReport(ReportRequestDTO request) throws IOException {
        log.debug("Генерация DOC отчета");
        ...
        log.info("DOC отчет успешно сгенерирован: {}, размер: {} bytes", reportTitle, outputStream.size());
        return outputStream.toByteArray();
    }
